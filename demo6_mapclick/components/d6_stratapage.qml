...
Rectangle {
    ...
    // Note: Removed automatic property watchers for currentPlotId and species_layer
    // updateForPlot() should only be called explicitly by the parent component
    // to avoid timing issues with plotFound state
            ...
            Loader {
                ...
                onLoaded: {
                    if (item) {
                        ...
                        console.error("d6_stratapage: Failed to load d6_species.qml")
                    }
                }
    ...
    function updateForPlot(plotId, found) {
        ...
        if (!(plotId && found)) {
            console.log("d6_stratapage (" + strataName + "): No plot selected or plot not found - hiding entries")
            return
        }
    ...
    function add_entry(){
        ...
        if (!species_layer) {
            console.error("d6_stratapage: Cannot add entry - species_layer is null")
            return
        }
        if (!currentPlotId) {
            console.error("d6_stratapage: Cannot add entry - currentPlotId is empty")
            return
        }
        ...
        if (!feature) {
            console.error("d6_stratapage: Failed to create new feature")
        ...
        if (!commitSuccess) {
            console.error("d6_stratapage: Failed to commit feature to database")
            species_layer.rollBack()
            return
        }
        ...
        if (feature_iterator.hasNext()) {
            ...
            try {
                ...
                console.error("d6_stratapage: Error creating new entry:", createError)
            }
        } else {
            console.error("d6_stratapage: Failed to retrieve committed feature")
            feature_iterator.close()
            return
        }
    }
    function create_widget(plotId, feature){
        // Validate inputs
        if (!feature) {
            console.error("d6_stratapage: Cannot create widget - feature is null")
            return
        }
        if (!plotId) {
            console.error("d6_stratapage: Cannot create widget - plotId is null")
            return
        }
        if (!entriesLayout) {
            console.error("d6_stratapage: Cannot create widget - entriesLayout is null")
            return
        }
        ...
        try {
            ...
            if (entryLoader == null) {
                console.error("d6_stratapage: Failed to create species entry component");
                return;
            }
            ...
            console.error("d6_stratapage: Error creating widget:", error)
        }
    }
    
    function remove_widget(entryLoader) {
        console.log("d6_stratapage: Removing widget from layout")
        ...
        console.log("d6_stratapage: Widget removed, remaining entries:", createdEntries.length)
    }
    function error(message){
        ...
        console.error("d6_stratapage: Error -", message)
    }
