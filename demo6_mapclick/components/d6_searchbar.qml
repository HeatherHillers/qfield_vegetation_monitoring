...
Rectangle {
    ...
    function setPlotId(plotId) {
        console.log("d6_searchbar: Setting plot ID in search bar:", plotId)
        selectByIdentifier(plotId)
        console.log("d6_searchbar:Plot ID set in search bar:", plotInput.currentText)
    ...
    function selectByIdentifier(targetId) {
        ...
        for (let i = 0; i < plot_model.count; i++) {
            ...
            if (plot_model.get(i).label === targetId) {
                ...
                console.log("d6_searchbar: Found '" + targetId + "' at index: " + i);
                console.log("d6_searchbar: Setting plotInput.currentIndex to:", i);
        ...
        console.log("d6_searchbar: Identifier '" + targetId + "' not found.");
    ...
    RowLayout {
        ...
        ComboBox {
            ...
            onCurrentIndexChanged: {
                // Use model data directly instead of currentText to avoid timing issues
                var plot_id = ""
                if (currentIndex >= 0 && currentIndex < plot_model.count) {
                    plot_id = plot_model.get(currentIndex).label
                }
                console.log("ComboBox current index changed to:", plot_id)
                search_plot_by_id(plot_id)
            }
            
            onAccepted: {
                // Handle manual text entry (when user types and presses Enter)
                search_plot()
            }

            function search_plot_by_id(plot_id){
            ...
            function search_plot(){
                // This function is kept for manual text entry (when user types)
